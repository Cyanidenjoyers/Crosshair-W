...
static gboolean draw_preview(GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    if (!w->enabled) {
        ...
        cairo_line_to(cr, alloc.width-5, alloc.height-5);
        cairo_move_to(cr, alloc.width-5, 5);
        cairo_line_to(cr, 5, alloc.height-5);
        cairo_stroke(cr);
        return FALSE;
    }
...
int main(int argc, char **argv) {
    ...
    // Catch Ctrl+C to clean up daemon
    ...
    gtk_widget_set_halign(widgets.color_button, GTK_ALIGN_START);
    ...
    widgets.thickness_label = gtk_label_new("Thickness:");
    ...
    gtk_css_provider_load_from_data(exit_provider,
        "button { background-color: #e5484d; color: white; font-weight: bold; font-size: 16px; border-radius: 8px; border: none; } "
        "button:hover { background-color: #f0585d; }", -1, NULL);
